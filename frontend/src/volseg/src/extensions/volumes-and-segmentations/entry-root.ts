...
import { ShapeGroup } from "molstar/lib/commonjs/mol-model/shape";
...
import { shallowEqualObjects } from "molstar/lib/commonjs/mol-util";
import { ParamDefinition } from "molstar/lib/commonjs/mol-util/param-definition";
import {
  isMeshlistData,
  MeshlistData,
  VolsegMeshSegmentation,
} from "../meshes/mesh-extension";
...
import {
  ...
  ShapePrimitiveData,
  TimeInfo,
} from "./volseg-api/data";
import {
  createSegmentKey,
  getSegmentLabelsFromDescriptions,
  instanceOfShapePrimitiveData,
  MetadataWrapper,
  parseSegmentKey,
} from "./volseg-api/utils";
import {
  DEFAULT_MESH_DETAIL,
  VolsegMeshSegmentationData,
} from "./entry-meshes";
...
import {
  ProjectDataParamsValues,
  ProjectGeometricSegmentationDataParamsValues,
  ProjectMeshSegmentationDataParamsValues,
  ProjectLatticeSegmentationDataParamsValues,
  type VolsegStateFromEntry,
} from "./transformers";
import { StateTransforms } from "molstar/lib/commonjs/mol-plugin-state/transforms";
import { Asset } from "molstar/lib/commonjs/mol-util/assets";
import { PluginComponent } from "molstar/lib/commonjs/mol-plugin-state/component";
import { VolsegGeometricSegmentationData } from "./entry-geometric-segmentation";
import {
  CreateShapePrimitiveProviderParamsValues,
  isShapePrimitiveParamsValues,
  VolsegGeometricSegmentation,
} from "./shape_primitives";
import { actionSelectSegment, parseCVSXJSON } from "../../common";
import { RuntimeContext } from "molstar/lib/commonjs//mol-task";
...
export const BOX: [[number, number, number], [number, number, number]] | null =
  null;
...
export function createLoadVolsegParams(
  ...
  const defaultVolumeServer =
    plugin?.config.get(VolsegVolumeServerConfig.DefaultServer) ??
  ...
  return ParamDefinition.Group(
    {
      ...
      customEntryId: ParamDefinition.Text("", {
        ...
        description:
          'Entry identifier, including the source prefix, e.g. "emd-1832"',
      }),
    },
    { isFlat: true },
  );
}
export type LoadVolsegParamValues = ParamDefinition.Values<
  ReturnType<typeof createLoadVolsegParams>
>;
...
type VolsegEntryParamValues = ParamDefinition.Values<
  ReturnType<typeof createVolsegEntryParams>
>;
...
export class VolsegEntry extends PluginStateObject.CreateBehavior<VolsegEntryData>(
  { name: "Vol & Seg Entry" },
) {}

export type VolRepr3DT =
  typeof StateTransforms.Representation.VolumeRepresentation3D;
...
class RawChannelData extends PluginComponent {
  constructor(
    public timeframeIndex: number,
    public channelId: string,
    public data: Uint8Array | string | RawMeshSegmentData[],
  ) {
    super();
  }
}
...
class RawTimeframesDataCache {
  private cache: Map<string, RawChannelData | RawSegmentationData> = new Map<
    string,
    RawChannelData | RawSegmentationData
  >();
  private maxEntries: number = 0;
  private totalSizeLimitInBytes: number = 1_000_000_000;

  constructor(
    public entryData: VolsegEntryData,
    public kind: RawDataKind,
  ) {}
  ...
  private _createKey(
    ...
    channelIdOrSegmentationId: string,
  ) {
    return `${timeframeIndex.toString()}_${channelIdOrSegmentationId}`;
  }

  private _getEntrySize(
    entry: RawChannelData | RawSegmentationData | RawMeshSegmentData,
  ) {
  ...
  async get(
    ...
    if (hasKey) {
      ...
      let entry;
      if (kind === "volume") {
        entry = await this.entryData._loadRawChannelData(
          timeframeIndex,
          channelIdOrSegmentationId,
        );
      } else if (kind === "segmentation") {
        entry = await this.entryData._loadRawLatticeSegmentationData(
          timeframeIndex,
          channelIdOrSegmentationId,
        );
      } else if (kind === "mesh") {
        entry = await this.entryData._loadRawMeshSegmentationData(
          timeframeIndex,
          channelIdOrSegmentationId,
        );
      } else if (kind === "primitive") {
        entry = await this.entryData._loadGeometricSegmentationData(
          timeframeIndex,
          channelIdOrSegmentationId,
        );
      } else {
...
export class VolsegEntryData extends PluginBehavior.WithSubscribers<VolsegEntryParamValues> {
  ...
  public cachedVolumeTimeframesData = new RawTimeframesDataCache(
    this,
    "volume",
  );
  public cachedSegmentationTimeframesData = new RawTimeframesDataCache(
    this,
    "segmentation",
  );
  public cachedMeshesTimeframesData = new RawTimeframesDataCache(this, "mesh");
  public cachedShapePrimitiveData = new RawTimeframesDataCache(
    this,
    "primitive",
  );
  state = {
  ...
  public readonly geometricSegmentationData =
    new VolsegGeometricSegmentationData(this);
  public readonly latticeSegmentationData = new VolsegLatticeSegmentationData(
    this,
  );
  ...
  private getStateNode = lazyGetter(
    () =>
      ...
      )[0] as StateObjectCell<
        VolsegState,
        StateTransform<typeof VolsegStateFromEntry>
      >,
    "Missing VolsegState node. Must first create VolsegState for this VolsegEntry.",
  );
  public currentState = new BehaviorSubject(
    ParamDefinition.getDefaultValues(VolsegStateParams),
  );
  ...
  private constructor(
    plugin: PluginContext,
    params: VolsegEntryParamValues,
    filesData?: CVSXFilesData,
  ) {
    super(plugin, params);
    this.plugin = plugin;
  ...
  private async initialize() {
    ...
    //   this.metadata.value!.raw.annotation?.entry_id.source_db_id ??
    //     this.entryId,
    // );
    await this.init();
  }
  ...
  private static getRawLatticeSegmentationsFromCSVX(
    cvsxFilesIndex: CVSXFilesIndex,
    zf: [string, Uint8Array][],
  ) {
    let rawLatticeSegmentations;
    if (cvsxFilesIndex.latticeSegmentations) {
      const condition = cvsxFilesIndex.latticeSegmentations;
      rawLatticeSegmentations = zf.filter((z) =>
        condition.hasOwnProperty(z[0]),
      );
    }
    return rawLatticeSegmentations;
  }

  private static getRawGeometricSegmentationsFromCVSX(
    cvsxFilesIndex: CVSXFilesIndex,
    zf: [string, Uint8Array][],
  ) {
    ...
    if (cvsxFilesIndex.geometricSegmentations) {
      ...
      rawGeometricSegmentations = zf.filter((z) =>
        condition.hasOwnProperty(z[0]),
      );
    }
  ...
  private static getRawMeshSegmentationsFromCSVX(
    cvsxFilesIndex: CVSXFilesIndex,
    zf: [string, Uint8Array][],
  ) {
  ...
  static async createFromFile(
    ...
    const zippedFiles: { [path: string]: Uint8Array } = (await unzip(
      runtimeCtx,
      file.buffer,
    )) as typeof zippedFiles;
    ...
    const cvsxFilesIndex: CVSXFilesIndex = await parseCVSXJSON(
      indexJsonEntry,
      plugin,
    );
    ...
    const annotationJSONEntry = zf.find(
      (z) => z[0] === cvsxFilesIndex.annotations,
    );

    const rawVolumes = zf.filter((z) =>
      cvsxFilesIndex.volumes.hasOwnProperty(z[0]),
    );

    const rawLatticeSegmentations = this.getRawLatticeSegmentationsFromCSVX(
      cvsxFilesIndex,
      zf,
    );
    const rawMeshSegmentations = this.getRawMeshSegmentationsFromCSVX(
      cvsxFilesIndex,
      zf,
    );
    const rawGeometricSegmentations = this.getRawGeometricSegmentationsFromCVSX(
      cvsxFilesIndex,
      zf,
    );
    ...
    if (!rawQueryJSON || !metadataJSONEntry || !annotationJSONEntry) {
      throw new Error(
        "CVSX has wrong content, some obligatory components are missing",
      );
    }

    // do parsing
    const parsedQueryJSON: QueryArgs = await parseCVSXJSON(
      rawQueryJSON,
      plugin,
    );
    ...
    const parsedGridMetadata: GridMetadata = await parseCVSXJSON(
      metadataJSONEntry,
      plugin,
    );
    const parsedAnnotationMetadata: AnnotationMetadata = await parseCVSXJSON(
      annotationJSONEntry,
      plugin,
    );
    ...
    const filesData: CVSXFilesData = {
      ...
      latticeSegmentations: cvsxData.latticeSegmentationDataFromRaw(
        rawLatticeSegmentations,
      ),
      meshSegmentations:
        cvsxData.meshSegmentationDataFromRaw(rawMeshSegmentations),
      geometricSegmentations: await cvsxData.geometricSegmentationDataFromRaw(
        rawGeometricSegmentations,
      ),
  ...
  async getData(
    ...
    if (kind === "volume") {
      const channelData = await this.cachedVolumeTimeframesData.get(
        timeframeIndex,
        channelIdOrSegmentationId,
        "volume",
      );
      ...
      const segmentationData = await this.cachedSegmentationTimeframesData.get(
        timeframeIndex,
        channelIdOrSegmentationId,
        "segmentation",
      );
      ...
      const meshData = await this.cachedMeshesTimeframesData.get(
        timeframeIndex,
        channelIdOrSegmentationId,
        "mesh",
      );
      ...
      const shapePrimitiveData = await this.cachedShapePrimitiveData.get(
        timeframeIndex,
        channelIdOrSegmentationId,
        "primitive",
      );
  ...
  sync() {
    ...
    const geometricSegmentations = this.findNodesByTags(
      GEOMETRIC_SEGMENTATION_NODE_TAG,
    );
  ...
  private async init() {
    ...
    const hasVolumes =
      this.metadata.value!.raw.grid.volumes.volume_sampling_info
        .spatial_downsampling_levels.length > 0;
    if (hasVolumes) {
    ...
    const hasShapePrimitives =
      this.metadata.value!.raw.grid.geometric_segmentation;
  ...
  private async initFromFile() {
    ...
    const hasGeometricSegmentation =
      this.metadata.value!.raw.grid.geometric_segmentation;
  ...
  async register(ref: string) {
    ...
    this.subscribeObservable(
      ...
      (e) => {
        ...
        if (e.cell.transform.ref === this.getStateNode().transform.ref) {
          ...
          if (
            newState &&
            !shallowEqualObjects(newState, this.currentState.value)
          ) {
    ...
    this.subscribeObservable(
      this.plugin.state.data.events.cell.removed,
      (e) => {
        if (volumeRefs.has(e.ref)) {
  ...
  async removeSegmentAnnotation(
    segmentId: number,
    segmentationId: string,
    kind: "lattice" | "mesh" | "primitive",
  ) {
    const targetAnnotation = this.metadata.value!.getSegmentAnnotation(
      segmentId,
      segmentationId,
      kind,
    );
    this.api.removeSegmentAnnotationsUrl(this.source, this.entryId, [
      targetAnnotation!.id,
    ]);
  ...
  async editDescriptions(descriptionData: DescriptionData[]) {
    await this.api.editDescriptionsUrl(
      this.source,
      this.entryId,
      descriptionData,
    );
  ...
  async editSegmentAnnotations(segmentAnnotationData: SegmentAnnotationData[]) {
    await this.api.editSegmentAnnotationsUrl(
      this.source,
      this.entryId,
      segmentAnnotationData,
    );
  ...
  async _loadRawMeshSegmentationData(
    timeframe: number,
    segmentationId: string,
  ) {
    const segmentsData: RawMeshSegmentData[] = [];
    const segmentsToCreate =
      this.metadata.value!.getMeshSegmentIdsForSegmentationIdAndTimeframe(
        segmentationId,
        timeframe,
      );
    for (const seg of segmentsToCreate) {
      const detail = this.metadata.value!.getSufficientMeshDetail(
        segmentationId,
        timeframe,
        seg,
        DEFAULT_MESH_DETAIL,
      );
      const urlString = this.api.meshUrl_Bcif(
        this.source,
        this.entryId,
        segmentationId,
        timeframe,
  ...
  async _loadGeometricSegmentationData(
    timeframe: number,
    segmentationId: string,
  ) {
    const url = this.api.geometricSegmentationUrl(
      this.source,
      this.entryId,
      segmentationId,
      timeframe,
  ...
  async _loadRawChannelData(timeframe: number, channelId: string) {
    const urlString = this.api.volumeUrl(
      this.source,
      this.entryId,
      timeframe,
      channelId,
  ...
  async _loadRawLatticeSegmentationData(
    timeframe: number,
    segmentationId: string,
  ) {
    const urlString = this.api.latticeUrl(
      this.source,
      this.entryId,
      segmentationId,
      timeframe,
  ...
  private async loadRawLatticeSegmentationData(
    timeInfoMapping: { [segmentation_id: string]: TimeInfo },
    ...
    for (const segmentationId of segmentationIds) {
      ...
      const end = timeInfo.end;
      for (let i = start; i <= end; i++) {
        const rawLatticeSegmentationData =
          await this._loadRawLatticeSegmentationData(i, segmentationId);
        this.cachedSegmentationTimeframesData.add(rawLatticeSegmentationData);
      }
    }
  }

  private async loadRawMeshSegmentationData(
    timeInfoMapping: { [segmentation_id: string]: TimeInfo },
    segmentationIds: string[],
  ) {
    for (const segmentationId of segmentationIds) {
      ...
      const end = timeInfo.end;
      for (let i = start; i <= end; i++) {
        const rawMeshSegmentationData = await this._loadRawMeshSegmentationData(
          i,
          segmentationId,
        );
  ...
  private async loadRawShapePrimitiveData(
    timeInfoMapping: { [segmentation_id: string]: TimeInfo },
    segmentationIds: string[],
  ) {
    for (const segmentationId of segmentationIds) {
      ...
      const end = timeInfo.end;
      for (let i = start; i <= end; i++) {
        const shapePrimitiveData = await this._loadGeometricSegmentationData(
          i,
          segmentationId,
        );
  ...
  preloadVolumeTimeframesDataFromFile() {
    ...
    for (const v of this.filesData!.volumes!) {
      const rawChannelData = new RawChannelData(
        v.timeframeIndex,
        v.channelId,
        v.data,
      );
  ...
  async preloadSegmentationTimeframesData() {
    if (this.metadata.value!.raw.grid.segmentation_lattices) {
      const segmentationIds =
        this.metadata.value!.raw.grid.segmentation_lattices.segmentation_ids;
      const timeInfoMapping =
        this.metadata.value!.raw.grid.segmentation_lattices.time_info;
  ...
  async preloadMeshesTimeframesData() {
    if (this.metadata.value!.raw.grid.segmentation_meshes) {
      const segmentationIds =
        this.metadata.value!.raw.grid.segmentation_meshes.segmentation_ids;
      const timeInfoMapping =
        this.metadata.value!.raw.grid.segmentation_meshes.time_info;
  ...
  async preloadShapePrimitivesTimeframesData() {
    if (this.metadata.value!.raw.grid.geometric_segmentation) {
      const segmentationIds =
        this.metadata.value!.raw.grid.geometric_segmentation.segmentation_ids;
      const timeInfoMapping =
        this.metadata.value!.raw.grid.geometric_segmentation.time_info;
  ...
  async updateProjectData(timeframeIndex: number) {
    ...
    const geometricSegmentations =
      this.state.hierarchy.value!.geometricSegmentations;
    ...
    for (const s of segmenations) {
      ...
      const oldParams: ProjectLatticeSegmentationDataParamsValues =
        s.transform.params;
      ...
      const segmentLabels = getSegmentLabelsFromDescriptions(
        descriptionsForLattice,
      );
      ...
      await this.plugin.state.updateTransform(
        this.plugin.state.data,
        projectSegmentationDataTransform,
        newParams,
        "Project Data Transform",
      );
    }
    ...
    for (const s of geometricSegmentations) {
      ...
      const oldParams: ProjectGeometricSegmentationDataParamsValues =
        s.transform.params;
    ...
    for (const m of meshSegmentations) {
      ...
      const oldParams: ProjectMeshSegmentationDataParamsValues =
        m.transform.params;
  ...
  async actionShowFittedModel(pdbIds: string[]) {
    ...
    await this.updateStateNode({
      visibleModels: pdbIds.map((pdbId) => ({ pdbId: pdbId })),
    });
  }

  async actionSetVolumeVisual(
    type: "isosurface" | "direct-volume" | "off",
    channelId: string,
    ...
    const channelToBeUpdated = currentChannelsData.filter(
  ...
  async actionUpdateVolumeVisual(
    params: SimpleVolumeParamValues,
    channelId: string,
    transform: StateTransform,
  ) {
  ...
  private async highlightSegment(segmentKey?: string) {
    ...
    if (segmentKey) {
      ...
      if (kind === "lattice") {
        await this.latticeSegmentationData.highlightSegment(
          segmentId,
          segmentationId,
        );
      } else if (kind === "mesh") {
        await this.meshSegmentationData.highlightSegment(
          segmentId,
          segmentationId,
        );
      } else if (kind === "primitive") {
        await this.geometricSegmentationData.highlightSegment(
          segmentId,
          segmentationId,
        );
      }
  ...
  private async selectSegment(segmentKey: string) {
    ...
    if (parsedSegmentKey.kind === "lattice") {
      await this.latticeSegmentationData.selectSegment(
        parsedSegmentKey.segmentId,
        parsedSegmentKey.segmentationId,
      );
    } else if (parsedSegmentKey.kind === "mesh") {
      await this.meshSegmentationData.selectSegment(
        parsedSegmentKey.segmentId,
        parsedSegmentKey.segmentationId,
      );
    } else if (parsedSegmentKey.kind === "primitive") {
      await this.geometricSegmentationData.selectSegment(
        parsedSegmentKey.segmentId,
        parsedSegmentKey.segmentationId,
      );
    }
  ...
  async updateStateNode(params: Partial<VolsegStateData>) {
    const nodes = this.plugin.state.data.selectQ((q) =>
      q.root.subtree().ofType(VolsegState),
    );
    if (nodes.length < 1) return;
    const stateNode = nodes[0];
    const oldParams = stateNode.transform.params;
    const newParams = { ...oldParams, ...params };
    const state = this.plugin.state.data;
    const update = state.build().to(stateNode.transform.ref).update(newParams);
  ...
  private readonly labelProvider: LociLabelProvider = {
    label: (loci: Loci): string | undefined => {
      ...
      if (segmentId === undefined || !segmentationId || !segmentationKind)
        return;
      const descriptions = this.metadata.value!.getSegmentDescription(
        segmentId,
        segmentationId,
        segmentationKind,
      );
      ...
      const annotLabels = descriptions[0].external_references?.map(
        (e) =>
          `${applyEllipsis(e.label ? e.label : "")} [${e.resource}:${e.accession}]`,
      );
      ...
      if (segmentName)
        onHoverLabel =
          onHoverLabel +
          '<hr class="msp-highlight-info-hr"/>' +
          "<b>" +
          segmentName +
          "</b>";
      ...
      if (onHoverLabel !== "") return onHoverLabel;
      else return;
    },
  };
  ...
  private getSegmentationIdFromLoci(loci: Loci): string | undefined {
    if (Volume.Segment.isLoci(loci)) {
      ...
      if (isMeshlistData(sourceData as any)) {
        ...
        const shapePrimitiveParamsValues = (loci.shape.sourceData ??
          {}) as CreateShapePrimitiveProviderParamsValues;
        return shapePrimitiveParamsValues.segmentationId;
      }
    }
  }

  private getSegmentationKindFromLoci(
    loci: Loci,
  ): "lattice" | "mesh" | "primitive" | undefined {
    if (Volume.Segment.isLoci(loci)) {
      return "lattice";
    } else if (ShapeGroup.isLoci(loci)) {
      const sourceData = loci.shape.sourceData;
  ...
  private getSegmentIdFromLoci(loci: Loci): number | undefined {
    if (
      Volume.Segment.isLoci(loci) &&
      loci.volume._propertyData.ownerId === this.ref
    ) {
