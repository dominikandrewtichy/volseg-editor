...
export class MetadataWrapper {
  ...
  hasLatticeSegmentations() {
    ...
    if (
      grid.segmentation_lattices &&
      grid.segmentation_lattices.segmentation_ids.length > 0
    ) {
  ...
  hasMeshSegmentations() {
    const grid = this.raw.grid;
    if (
      grid.segmentation_meshes &&
      grid.segmentation_meshes.segmentation_ids.length > 0
    ) {
  ...
  hasGeometricSegmentations() {
    const grid = this.raw.grid;
    if (
      grid.geometric_segmentation &&
      grid.geometric_segmentation.segmentation_ids.length > 0
    ) {
  ...
  getDescriptions(
    ...
    const allDescriptonsForSegmentationId = allDescriptions.filter(
      (d) =>
        d.target_id &&
        d.target_id.segmentation_id === segmentationId &&
        d.target_kind === kind,
    );
    const allDescriptonsForSegmentationIdAndTimeframe =
      allDescriptonsForSegmentationId.filter((a) => {
        if (a.hasOwnProperty("time") && Number.isFinite(a.time)) {
          ...
          a.time.every((i) => Number.isFinite(i))
        ) {
  ...
  getVolumeChannelColor(channel_id: string) {
    ...
    const channelColorArray = this.channelAnnotations.filter(
      (i) => i.label === channel_id,
    )[0]?.color;
  ...
  getVolumeChannelLabel(channel_id: string) {
    ...
    const volumeChannelLabel = this.channelAnnotations.filter(
  ...
  getSegmentAnnotation(
    segmentId: number,
    segmentationId: string,
    kind: "lattice" | "mesh" | "primitive",
  ) {
    const allAnnotations = this.allAnnotations;
    return allAnnotations.find(
      (a) =>
        a.segment_id === segmentId &&
        a.segment_kind === kind &&
        a.segmentation_id === segmentationId,
    );
  }
      ...
      if (!this.segmentMap) {
        ...
        for (const description of this.allDescriptions) {
          if (description.target_id && description.target_kind !== "entry") {
            ...
            const _segmentKey = createSegmentKey(
              _segmentId,
              _segmentationId,
              _kind,
            );
            ...
            if (existingDescriptions) {
              this.segmentMap.set(_segmentKey, [
  ...
  getMeshDetailLevels(
    ...
    const meshComponentNumbers =
      this.raw.grid.segmentation_meshes.segmentation_metadata[segmentationId]
        .mesh_timeframes[timeframe];
  ...
  getSufficientMeshDetail(
    segmentationId: string,
    timeframe: number,
    segmentId: number,
    preferredDetail: number | null,
  ) {
    let availDetails = this.getMeshDetailLevels(
      segmentationId,
      timeframe,
      segmentId,
    );
  ...
  getMeshSegmentIdsForSegmentationIdAndTimeframe(
    segmentationId: string,
    timeframe: number,
  ) {
    // if (!this.raw.grid.segmentation_meshes) return;
  ...
  getAllSegmentAnotationsForSegmentationAndTimeframe(
    ...
    const allAnnotations = this.allAnnotations;
    const allAnnotationsForSegmentationId = allAnnotations.filter(
      (d) => d.segmentation_id === segmentationId && d.segment_kind === kind,
    );
    const allAnnotationsForSegmentationIdAndTimeframe =
      allAnnotationsForSegmentationId.filter((a) => {
        if (a.hasOwnProperty("time") && Number.isFinite(a.time)) {
          if (timeframeIndex === a.time) {
            return a;
          }
  ...
  get gridTotalVolume() {
    ...
    const [vx, vy, vz] =
      this.raw.grid.volumes.volume_sampling_info.boxes[
        currentVolumeDownsampling
      ].voxel_size;
    const [gx, gy, gz] =
      this.raw.grid.volumes.volume_sampling_info.boxes[
        currentVolumeDownsampling
...
export function instanceOfShapePrimitiveData(
  object: any,
): object is ShapePrimitiveData {
...
export function getSegmentLabelsFromDescriptions(
  descriptions: DescriptionData[],
) {
  return descriptions.map((description) => ({
    id: description.target_id!.segment_id,
    label:
      description?.name && !description?.is_hidden
        ? `<b>${description?.name}</b>`
        : "No label provided or description is hidden",
  }));
}
