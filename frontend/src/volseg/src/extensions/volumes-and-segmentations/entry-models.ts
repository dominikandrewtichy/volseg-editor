...
export class VolsegModelData {
  ...
  private async loadPdb(
    pdbId: string,
    parent: StateObjectSelector | StateObjectRef,
  ) {
    const url = `https://www.ebi.ac.uk/pdbe/entry-files/download/${pdbId}.bcif`;
    const dataNode = await this.entryData.plugin
      .build()
      .to(parent)
      .apply(
        Download,
        { url: url, isBinary: true },
        { tags: ["fitted-model-data", `pdbid-${pdbId}`] },
      )
      .commit();
    const cifNode = await this.entryData.plugin
      .build()
      .to(dataNode)
      .apply(ParseCif)
      .commit();
    const trajectoryNode = await this.entryData.plugin
      .build()
      .to(cifNode)
      .apply(TrajectoryFromMmCif)
      .commit();
    await this.entryData.plugin.builders.structure.hierarchy.applyPreset(
      trajectoryNode,
      "default",
  ...
  async showPdbs(pdbIds: string[]) {
    ...
    let group = this.entryData.findNodesByTags("fitted-models-group")[0]
      ?.transform.ref;
    if (!group) {
      const newGroupNode = await this.entryData
        ...
        .apply(
          CreateGroup,
          { label: "Fitted Models" },
          { tags: ["fitted-models-group"], state: { isCollapsed: true } },
        )
        .commit();
