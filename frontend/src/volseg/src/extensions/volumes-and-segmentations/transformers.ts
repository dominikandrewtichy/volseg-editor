...
import { DensityServer_Data_Schema } from "molstar/lib/commonjs/mol-io/reader/cif/schema/density-server";
import { volumeFromDensityServerData } from "molstar/lib/commonjs/mol-model-formats/volume/density-server";
import { volumeFromSegmentationData } from "molstar/lib/commonjs/mol-model-formats/volume/segmentation";
...
import { StateTransformer } from "molstar/lib/commonjs/mol-state";
...
export const ProjectLatticeSegmentationDataParams = {
  ...
  segmentLabels: ParamDefinition.ObjectList(
    { id: ParamDefinition.Numeric(-1), label: ParamDefinition.Text("") },
    (s) => `${s.id} = ${s.label}`,
    { description: "Mapping of segment IDs to segment labels" },
  ),
  ownerId: ParamDefinition.Text("", {
    isHidden: true,
    description: "Reference to the object which manages this volume",
  }),
};
...
export type ProjectDataParamsValues = ParamDefinition.Values<
  typeof ProjectDataParams
>;
export type ProjectLatticeSegmentationDataParamsValues = ParamDefinition.Values<
  typeof ProjectLatticeSegmentationDataParams
>;
export type ProjectMeshSegmentationDataParamsValues = ParamDefinition.Values<
  typeof ProjectMeshSegmentationDataParams
>;
...
export const ProjectVolumeData = CreateTransformer({
  ...
  apply({ a, params, spine }, plugin: PluginContext) {
    return Task.create("Project Volume Data", async (ctx) => {
      ...
      const queryResults = plugin.state.data.selectQ((q) =>
        q.ofType(VolsegEntry),
      );
      if (queryResults.length < 1) return;
      const entry = queryResults[0].obj;
      if (!entry) return;
      // const entry = spine.getAncestorOfType(VolsegEntry);
      const entryData = entry!.data;
      const rawData = (await entryData.getData(
        timeframeIndex,
        channelId,
        "volume",
      ...
      const volume =
        await volumeFromDensityServerData(volumeCif).runInContext(ctx);
      const [x, y, z] = volume.grid.cells.space.dimensions;
      const props = {
        label: `Volume channel: ${label}`,
        description: `Volume ${x}\u00D7${y}\u00D7${z}`,
      };
      const volumedata3dinfo = volume.sourceData
        .data as DensityServer_Data_Schema;
      const samplerate: any = volumedata3dinfo.volume_data_3d_info.sample_rate;
      const downsampling: number = samplerate.__array[0];
      entryData.metadata.value!.setCurrentlyUsedVolumeDownsampling(
        downsampling,
      );
...
export const ProjectSegmentationData = CreateTransformer({
  ...
  display: {
    name: "Project Segmentation Data",
    description: "Project Segmentation Data",
  },
...
export const ProjectMeshData = CreateTransformer({
  ...
  apply({ a, params, spine }, plugin: PluginContext) {
    return Task.create("Project Mesh Data", async (ctx) => {
      ...
      const rawDataArray: RawMeshSegmentData[] = (await entryData.getData(
        timeframeIndex,
        segmentationId,
        "mesh",
      )) as RawMeshSegmentData[];
      for (const segmentParam of segmentsParams) {
        const rawDataItem = rawDataArray.find(
          (i) => i.segmentId === segmentParam.id,
        );
      ...
      return new VolsegMeshSegmentation(new VolsegMeshData(meshData), {
        label: `Segmentation ID: ${segmentationId}`,
      });
    });
  },
});
...
export type ProjectGeometricSegmentationDataParamsValues =
  ParamDefinition.Values<typeof ProjectGeometricSegmentationDataParams>;

export const ProjectGeometricSegmentationData = CreateTransformer({
  name: "project-geometric-segmentation",
  display: {
    name: "Project Geometric Segmentation",
    description: "Project Geometric Segmentation",
  },
  from: VolsegEntry,
  ...
  apply({ a, params, spine }, plugin: PluginContext) {
    return Task.create("Project Geometric Segmentation Data", async (ctx) => {
      ...
      const entry = spine.getAncestorOfType(VolsegEntry);
      const entryData = entry!.data;
      const shapePrimitiveData = (await entryData.getData(
        timeframeIndex,
        segmentationId,
        "primitive",
      )) as ShapePrimitiveData;
      return new VolsegGeometricSegmentation(
        new VolsegShapePrimitivesData(shapePrimitiveData),
        { label: `Segmentation ID: ${segmentationId}` },
      );
    });
  },
...
export const VolsegEntryFromRoot = CreateTransformer({
  ...
  apply({ a, params }, plugin: PluginContext) {
    return Task.create("Load Vol & Seg Entry", async () => {
      ...
      return new VolsegEntry(data, {
        label: data.entryId,
        description: "Vol & Seg Entry",
      });
    });
  },
  update({ b, oldParams, newParams }) {
    ...
    console.error(
      "Changing params of existing VolsegEntry node is not allowed",
    );
...
export const VolsegStateFromEntry = CreateTransformer({
  ...
  display: {
    name: "Vol & Seg Entry State",
    description: "Vol & Seg Entry State",
  },
  from: VolsegEntry,
  to: VolsegState,
...
export const VolsegGlobalStateFromFile = CreateTransformer({
  ...
  display: {
    name: "Vol & Seg Global State",
    description: "Vol & Seg Global State",
  },
  from: PluginStateObject.Data.Binary,
  ...
  apply({ a, params }, plugin: PluginContext) {
    return Task.create("Create Vol & Seg Global State", async () => {
      ...
      return new VolsegGlobalState(data, {
        label: "Global State",
        description: "Vol & Seg Global State",
      });
    });
