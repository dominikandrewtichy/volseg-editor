...
import { ColorNames } from "molstar/lib/commonjs/mol-util/color/names";
...
import {
  ...
  CreateMeshlistStateObject,
  FOREROUND_OPACITY,
  MeshShapeTransformer,
  MeshlistData,
  VolsegMeshSegmentation,
  meshSegmentParamsValues,
...
export class VolsegMeshSegmentationData {
  ...
  async createMeshRepresentation3D(
    meshNode: StateObjectSelector<VolsegMeshSegmentation>,
    params: ProjectMeshSegmentationDataParamsValues,
  ) {
    ...
    for (const meshDataItem of meshData) {
      ...
      const meshListStateObj = await update
        .to(meshNode)
      ...
      await this.entryData.plugin
        .build()
        .to(meshListStateObj)
        .apply(MeshShapeTransformer, {
          color: meshDataItem.meshSegmentParams.color,
        })
        .apply(
          ...
          { alpha: transparent ? BACKGROUND_OPACITY : FOREROUND_OPACITY },
          {
            tags: [
              "mesh-segment-visual",
              `segment-${meshDataItem.meshSegmentParams.id}`,
              segmentationId,
            ],
          },
        )
  ...
  getMeshSegmentParams(segmentationId: string, timeframeIndex: number) {
    ...
    const segmentsToCreate =
      this.entryData.metadata.value!.getMeshSegmentIdsForSegmentationIdAndTimeframe(
        segmentationId,
        timeframeIndex,
      );
    const segmentAnnotations =
      this.entryData.metadata.value!.getAllSegmentAnotationsForSegmentationAndTimeframe(
        segmentationId,
        "mesh",
        timeframeIndex,
      );
    for (const seg of segmentsToCreate) {
      const colorData = segmentAnnotations.find(
        (a) => a.segment_id === seg,
      )?.color;
      const color =
        colorData && colorData.length >= 3
          ? Color.fromNormalizedArray(colorData, 0)
          : ColorNames.gray;
      const label = `${segmentationId} | Segment ${seg}`;

      const detail = this.entryData.metadata.value!.getSufficientMeshDetail(
        segmentationId,
        timeframeIndex,
        seg,
        DEFAULT_MESH_DETAIL,
  ...
  async createMeshGroup() {
    let group = this.entryData.findNodesByTags("mesh-segmentation-group")[0]
      ?.transform.ref;
    if (!group) {
      const newGroupNode = await this.entryData
        ...
        .apply(
          CreateGroup,
          { label: "Segmentation", description: "Mesh" },
          { tags: ["mesh-segmentation-group"], state: { isCollapsed: true } },
        )
        .commit();
  ...
  async highlightSegment(segmentId: number, segmentationId: string) {
    ...
    for (const visual of visuals) {
      await PluginCommands.Interactivity.Object.Highlight(
        this.entryData.plugin,
        { state: this.entryData.plugin.state.data, ref: visual.transform.ref },
      );
    }
  }
