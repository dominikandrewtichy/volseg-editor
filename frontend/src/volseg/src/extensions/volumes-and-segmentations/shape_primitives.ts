...
import { MeshBuilder } from "molstar/lib/commonjs/mol-geo/geometry/mesh/mesh-builder";
import { addSphere } from "molstar/lib/commonjs/mol-geo/geometry/mesh/builder/sphere";
import { Mat4, Vec3 } from "molstar/lib/commonjs/mol-math/linear-algebra";
...
import { addCylinder } from "molstar/lib/commonjs/mol-geo/geometry/mesh/builder/cylinder";
import { Box } from "molstar/lib/commonjs/mol-geo/primitive/box";
import {
  Pyramid,
  TriangularPyramid,
} from "molstar/lib/commonjs/mol-geo/primitive/pyramid";
import { Primitive } from "molstar/lib/commonjs/mol-geo/primitive/primitive";
import { polygon } from "molstar/lib/commonjs/mol-geo/primitive/polygon";
import { addEllipsoid } from "molstar/lib/commonjs/mol-geo/geometry/mesh/builder/ellipsoid";
import {
  DescriptionData,
...
export class VolsegGeometricSegmentation extends PluginStateObject.Create<VolsegShapePrimitivesData>(
  { name: "Vol & Seg Geometric Segmentation", typeClass: "Data" },
) {}
...
export const isShapePrimitiveParamsValues = (
  value: CreateShapePrimitiveProviderParamsValues,
): value is CreateShapePrimitiveProviderParamsValues =>
  !!value?.segmentAnnotations;

export type CreateShapePrimitiveProviderParamsValues = PD.Values<
  typeof CreateShapePrimitiveProviderParams
>;
export const CreateShapePrimitiveProviderParams = {
  segmentAnnotations: PD.Value<SegmentAnnotationData[]>([] as any, {
    isHidden: true,
  }),
    ...
    return new PluginStateObject.Shape.Provider(
      {
        ...
        getShape: (_, data) =>
          createShapePrimitive(a.data.shapePrimitiveData, params),
      },
      { label: "Shape Primitives" },
    );
  },
...
function _get_target_segment_name(
  allDescriptions: DescriptionData[],
  segment_id: number,
) {
  // NOTE: for now single description
  const description = allDescriptions.filter(
    (d) => d.target_id && d.target_id.segment_id === segment_id,
  );
  return description[0].name!;
}

export function _get_target_segment_color(
  allSegmentAnnotations: SegmentAnnotationData[],
  segment_id: number,
) {
  // NOTE: for now single annotation, should be single one
  const annotation = allSegmentAnnotations.find(
    (a) => a.segment_id === segment_id,
  );
...
function createShapePrimitive(
  ...
  params: CreateShapePrimitiveProviderParamsValues,
) {
  ...
  switch (p!.kind) {
    ...
    case "cylinder":
      addCylinder(
        builder,
        (p as Cylinder).start as Vec3,
        (p as Cylinder).end as Vec3,
        1,
        {
  ...
  return Shape.create(
    ...
    (g) =>
      Color.fromNormalizedArray(
        _get_target_segment_color(segmentAnnotations, p!.id),
        0,
      ),
    () => 1,
