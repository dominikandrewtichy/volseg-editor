...
export const VolumeTypeChoice = new Choice(
  {
...
export const VolsegStateParams = {
  ...
  visibleModels: PD.ObjectList({ pdbId: PD.Text("") }, (s) =>
    s.pdbId.toString(),
  ),
  channelsData: PD.ObjectList(
    {
      channelId: PD.Text("0"),
      volumeIsovalueKind: PD.Select("relative", [
        ["relative", "Relative"],
        ["absolute", "Absolute"],
      ]),
...
export class VolsegState extends PluginStateObject.Create<VolsegStateData>({
  name: "Vol & Seg Entry State",
  typeClass: "Data",
}) {}

export const VOLSEG_STATE_FROM_ENTRY_TRANSFORMER_NAME =
  "volseg-state-from-entry"; // defined here to avoid cyclic dependency
