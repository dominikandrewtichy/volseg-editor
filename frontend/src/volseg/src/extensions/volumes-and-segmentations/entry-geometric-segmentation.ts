...
import {
  CreateShapePrimitiveProvider,
  VolsegGeometricSegmentation,
  VolsegShapePrimitivesData,
  _get_target_segment_color,
...
export class VolsegGeometricSegmentationData {
  ...
  async createGeometricSegmentationGroup() {
    let group = this.entryData.findNodesByTags(
      GEOMETRIC_SEGMENTATION_GROUP_TAG,
    )[0]?.transform.ref;
    if (!group) {
      const newGroupNode = await this.entryData
        ...
        .apply(
          CreateGroup,
          { label: "Segmentation", description: "Geometric segmentation" },
          {
            tags: [GEOMETRIC_SEGMENTATION_GROUP_TAG],
  ...
  async createGeometricSegmentationRepresentation3D(
    gsNode: StateObjectSelector<VolsegGeometricSegmentation>,
    params: ProjectGeometricSegmentationDataParamsValues,
  ) {
    const gsData: VolsegShapePrimitivesData = gsNode.cell!.obj!.data;
    const { timeframeIndex, segmentationId } = params;
    const descriptions = this.entryData.metadata.value!.getDescriptions(
      segmentationId,
      "primitive",
      timeframeIndex,
    );
    ...
    for (const primitiveData of gsData.shapePrimitiveData
      .shape_primitive_list) {
      const color = _get_target_segment_color(
        segmentAnnotations,
        primitiveData.id,
      );
      const opacity = color[3];
      update
        .apply(CreateShapePrimitiveProvider, {
          segmentId: primitiveData.id,
