...
import { StateObjectSelector } from "molstar/lib/commonjs/mol-state";
...
export class VolsegLatticeSegmentationData {
  ...
  async createSegmentationGroup() {
    ...
    if (!group) {
      const newGroupNode = await this.entryData
        ...
        .apply(
          CreateGroup,
          { label: "Segmentation", description: "Lattice" },
  ...
  async createSegmentationRepresentation3D(
    segmentationNode: StateObjectSelector<PluginStateObject.Volume.Data>,
    params: ProjectLatticeSegmentationDataParamsValues,
  ) {
      ...
      .apply(
        ...
        createVolumeRepresentationParams(
          ...
          segmentationData,
          {
            type: "segment",
            typeParams: {
              tryUseGpu: VolsegGlobalStateData.getGlobalState(
                this.entryData.plugin,
              )?.tryUseGpu,
            },
            color: "volume-segment",
            colorParams: {
              palette: this.getPaletteForSegmentation(
                segmentationId,
                segmentIds,
              ),
            },
        ...
        { tags: [SEGMENT_VISUAL_TAG, segmentationId] },
      )
      .commit();
  }
  ...
  private getPaletteForSegmentation(
    ...
    if (!colorMapForSegmentation) {
      ...
      return {
        ...
        params: { list: { kind: "set" as const, colors: colors } },
      };
    } else {
      const colors = segmentIds.map(
        (segid) => colorMapForSegmentation.get(segid) ?? DEFAULT_SEGMENT_COLOR,
      );
      return {
        name: "colors" as const,
        params: { list: { kind: "set" as const, colors: colors } },
      ...
      for (const annotation of this.entryData.metadata.value!.allAnnotations) {
        if (annotation.color) {
          ...
          if (colorMapForAllSegmentations.get(annotation.segmentation_id)) {
            ...
            const colorMapForSegmentation: Map<number, Color> =
              colorMapForAllSegmentations.get(annotation.segmentation_id);
            colorMapForSegmentation.set(annotation.segment_id, color);
            colorMapForAllSegmentations.set(
              annotation.segmentation_id,
              colorMapForSegmentation,
            );
          } else {
  ...
  async updateOpacity(opacity: number, segmentationId: string) {
    const s = this.entryData.findNodesByTags(
      SEGMENT_VISUAL_TAG,
      segmentationId,
    )[0];
  ...
  private makeLoci(segments: number[], segmentationId: string) {
    const vis = this.entryData.findNodesByTags(
      SEGMENT_VISUAL_TAG,
      segmentationId,
    )[0];
  ...
  async highlightSegment(segmentId: number, segmentationId: string) {
    ...
    this.entryData.plugin.managers.interactivity.lociHighlights.highlight(
      segmentLoci,
      false,
    );
  }
  ...
  async showSegments(segmentIds: number[], segmentationId: string) {
    const repr = this.entryData.findNodesByTags(
      SEGMENT_VISUAL_TAG,
      segmentationId,
    )[0];
    if (!repr) return;
    const selectedSegmentKey =
      this.entryData.currentState.value.selectedSegment;
