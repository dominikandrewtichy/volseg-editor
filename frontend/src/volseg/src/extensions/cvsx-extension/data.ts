...
import {
  ...
  GridMetadata,
  ShapePrimitiveData,
} from "../volumes-and-segmentations/volseg-api/data";
...
export interface CVSXFilesIndex {
  ...
  geometricSegmentations?: {
    [filename: string]: GeometricSegmentationFileInfo;
  };
...
export class CVSXData {
  constructor(
    public cvsxFilesIndex: CVSXFilesIndex,
  ...
  meshSegmentationDataFromRaw(rawData?: [string, Uint8Array][]) {
    ...
    for (const m of meshesInfo) {
      const targetSegments = rawData.filter((r) =>
        m.segmentsFilenames.includes(r[0]),
      );
  ...
  async geometricSegmentationDataFromRaw(rawData?: [string, Uint8Array][]) {
    ...
    for (const gsFileName in gsInfo) {
      ...
      const parsedGsData: ShapePrimitiveData = await parseCVSXJSON(
        gsData,
        this.plugin,
      );
