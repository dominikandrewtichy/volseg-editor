...
import { CIF, CifFile } from "molstar/lib/commonjs/mol-io/reader/cif";
...
import { Grid } from "molstar/lib/commonjs/mol-model/volume";
import { ColorNames } from "molstar/lib/commonjs/mol-util/color/names";
import { TypedArray } from "molstar/lib/commonjs/mol-util/type-helpers";
...
export function concat(...meshes: Mesh[]): Mesh {
  ...
  const vertices = concatArrays(
    Float32Array,
    meshes.map((m) => m.vertexBuffer.ref.value),
  );
  const normals = concatArrays(
    Float32Array,
    meshes.map((m) => m.normalBuffer.ref.value),
  );
  const groups = concatArrays(
    Float32Array,
    meshes.map((m) => m.groupBuffer.ref.value),
  );
  const newIndices = [];
  let offset = 0;
...
export async function meshFromCif(
  ...
  if (outSpace === "cartesian") {
    ...
    const dimensionFract =
      volumeInfoCif.volume_data_3d_info.dimensions.value(0);
    if (
      dimensionFract[0] !== 1 ||
      dimensionFract[1] !== 1 ||
      dimensionFract[2] !== 1
      ...
      throw new Error(
        `Asserted the fractional dimensions are [1,1,1], but are actually [${dimensionFract}]`,
      );
...
/** Return bounding box */
export function bbox(mesh: Mesh): Box3D | null {
  ...
  return Box3D.create(
    Vec3.create(minX, minY, minZ),
    Vec3.create(maxX, maxY, maxZ),
  );
}
...
export function meshFromBox(
  ...
  const indices = new Uint32Array([
    2, 1, 0, 1, 2, 3, 1, 4, 0, 4, 1, 5, 3, 5, 1, 5, 3, 7, 2, 7, 3, 7, 2, 6, 0,
    6, 2, 6, 0, 4, 4, 7, 6, 7, 4, 5,
  ]);
