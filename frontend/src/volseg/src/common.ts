...
import { Choice } from "molstar/lib/commonjs/mol-util/param-choice";
...
import {
  ...
  ParsedSegmentKey,
  SegmentPointers,
} from "./extensions/volumes-and-segmentations/volseg-api/data";
import {
  createSegmentKey,
  parseSegmentKey,
} from "./extensions/volumes-and-segmentations/volseg-api/utils";

export async function parseCVSXJSON(
...
export async function actionToggleAllFilteredSegments(
  model: VolsegEntryData,
  segmentationId: string,
  kind: "lattice" | "mesh" | "primitive",
  filteredDescriptions: DescriptionData[],
) {
  const current = model.currentState.value.visibleSegments.map(
    (seg) => seg.segmentKey,
  );
  const currentForThisSegmentation = current.filter(
    (c) =>
      parseSegmentKey(c).segmentationId === segmentationId &&
      parseSegmentKey(c).kind === kind,
  );
  const currentForOtherSegmentations = current.filter(
    (item) => currentForThisSegmentation.indexOf(item) < 0,
  );
  const allFilteredSegmentKeys = filteredDescriptions
    .map((d) => {
      if (d.target_kind !== "entry") {
        return createSegmentKey(
          d.target_id!.segment_id,
          d.target_id!.segmentation_id,
          d.target_kind,
        );
      }
    })
  ...
  if (currentForThisSegmentation.length !== filteredDescriptions.length) {
    const allSegmentKeys = [
      ...allFilteredSegmentKeys,
      ...currentForOtherSegmentations,
    ];
...
export async function actionShowSegments(
  segmentKeys: string[],
  model: VolsegEntryData,
) {
  const allExistingLatticeSegmentationIds =
    model.metadata.value!.raw.grid.segmentation_lattices!.segmentation_ids;
  const allExistingMeshSegmentationIds =
    model.metadata.value!.raw.grid.segmentation_meshes!.segmentation_ids;
  const allExistingGeometricSegmentationIds =
    model.metadata.value!.raw.grid.geometric_segmentation!.segmentation_ids;
  if (segmentKeys.length === 0) {
    for (const id of allExistingLatticeSegmentationIds) {
      await showSegments(
        { segmentIds: [], segmentationId: id, kind: "lattice" },
        model,
      );
    }
    for (const id of allExistingMeshSegmentationIds) {
      await showSegments(
        { segmentIds: [], segmentationId: id, kind: "mesh" },
        model,
      );
    }
    for (const id of allExistingGeometricSegmentationIds) {
      await showSegments(
        { segmentIds: [], segmentationId: id, kind: "primitive" },
        model,
      );
    }
  }
  const parsedSegmentKeys = segmentKeys.map((k) => parseSegmentKey(k));
  // LATTICES PART
  _actionShowSegments(
    parsedSegmentKeys,
    allExistingLatticeSegmentationIds,
    "lattice",
    model,
  );
  // MESHES PART
  _actionShowSegments(
    parsedSegmentKeys,
    allExistingMeshSegmentationIds,
    "mesh",
    model,
  );
  ...
  _actionShowSegments(
    parsedSegmentKeys,
    allExistingGeometricSegmentationIds,
    "primitive",
    model,
  );

  await model.updateStateNode({
    visibleSegments: segmentKeys.map((s) => ({ segmentKey: s })),
  });
}

export async function _actionShowSegments(
  parsedSegmentKeys: ParsedSegmentKey[],
  existingSegmentationIds: string[],
  kind: "mesh" | "lattice" | "primitive",
  model: VolsegEntryData,
) {
  ...
  for (const key of segmentKeys) {
    if (!SegmentationIdsToSegmentIds.has(key.segmentationId)) {
      ...
      const currentSegmentationIds = SegmentationIdsToSegmentIds.get(
        key.segmentationId,
      );
      SegmentationIdsToSegmentIds.set(key.segmentationId, [
        ...currentSegmentationIds!,
        key.segmentId,
      ]);
    }
  }
  ...
  SegmentationIdsToSegmentIds.forEach((value, key) => {
    promises.push(
      showSegments(
        { segmentIds: value, segmentationId: key, kind: kind },
        model,
      ),
    );
  });
...
async function showSegments(
  segmentPointers: SegmentPointers,
  model: VolsegEntryData,
) {
  const { segmentIds, segmentationId, kind } = segmentPointers;
  if (kind === "lattice") {
    const repr = findNodesByTags(
      model.plugin,
      SEGMENT_VISUAL_TAG,
      segmentationId,
    )[0];
    ...
    const selectedSegmentSegmentationId =
      parsedSelectedSegmentKey.segmentationId;
    const mustReselect =
      segmentIds.includes(selectedSegmentId) &&
      !repr.params?.values.type.params.segments.includes(selectedSegmentId);
    const update = model.plugin.build().toRoot();
    update
      .to(repr)
      .update(StateTransforms.Representation.VolumeRepresentation3D, (p) => {
        p.type.params.segments = segmentIds;
      });
    await update.commit();
    if (mustReselect) {
      await selectSegment(
        model,
        selectedSegmentId,
        selectedSegmentSegmentationId,
      );
    }
  } else if (kind === "mesh") {
    const segmentsToShow = new Set(segmentIds);

    const visuals = findNodesByTags(
      model.plugin,
      "mesh-segment-visual",
      segmentationId,
    );
    for (const visual of visuals) {
      const theTag = visual.obj?.tags?.find((tag) =>
...
export async function actionSelectSegment(
  model: VolsegEntryData,
  segmentKey?: string,
) {
  if (
    segmentKey !== undefined &&
    model.currentState.value.visibleSegments.find(
      (s) => s.segmentKey === segmentKey,
    ) === undefined
  ) {
...
export async function actionToggleSegment(
  model: VolsegEntryData,
  segmentKey: string,
) {
  const current = model.currentState.value.visibleSegments.map(
  ...
  if (current.includes(segmentKey)) {
    await actionShowSegments(
      current.filter((s) => s !== segmentKey),
      model,
    );
  } else {
...
export const SourceChoice = new Choice(
  {
    emdb: "EMDB",
    empiar: "EMPIAR",
    idr: "IDR",
